...
import { BookServices2 } from "../services/BookServices2";
...
const Booklisting = () => {
    ...
    const getAllBooks = async () => {
        await BookServices2().then((Response) => {
    ...
    const handleDelete = async (rId) => {
        ...
        await DeleteBook(Number(rId)).then((Response) => {
            if (Response && Response.status === 200) {
                toast.success("Book deleted successfully", { position: 'bottom-right' });
                getBooks(filter);
            }
        }).catch(() => {
            toast.error("Book can't be deleted", { position: 'bottom-right' })
        })
    }
                                    ...
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>
                                            <Button variant="outlined" disableElevation
                                                ...
                                                }} onClick={() => { navigate(`/edit-books/${row.id}`) }}
                                            >
                                                Edit
                                            </Button>
                                        </TableCell>
...
export default WithAuth(Booklisting);
